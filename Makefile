...
VENV_RECORD_DIR = $(CURDIR)/data

all: install

$(VENV_DIR): requirements-dev.txt
	$(VENV) $(VENV_ARGS) "$(VENV_DIR)"
	echo "export RECORD_DIR=\"$(VENV_RECORD_DIR)\"" >> "$(VENV_DIR)"/bin/activate
	echo "set -x RECORD_DIR \"$(VENV_RECORD_DIR)\"" >> "$(VENV_DIR)"/bin/activate.fish
