...
from record.config import ConfigParser

from . import mock_read


def test_config_get(mocker, record):
    content = """
[backend]
url = foo
token =
...
def test_config_getboolean(mocker, record):
    content = """
[options]
flag1 = 1
flag2 = ON
...
def test_config_getint(mocker, record):
    content = """
[options]
value1 = 42
value2 = spamm
...
def test_config_getfloat(mocker, record):
    content = """
[options]
value1 = 3.14
value2 = 42
...
def test_config_getlist(mocker, record):
    content = """
...
def test_set_config(record):
    config = ConfigParser()
    config.set('foo', 'bar', 'lol')
    record.config = config

    assert record.config.get('foo', 'bar') == 'lol'
